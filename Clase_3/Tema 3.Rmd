...
poblacion <- 1000     # Población inicial
objetivo <- 50000
horas <- 0
...
TMB <- function(Sexo,Altura,Peso,Edad){
  if(Sexo=="mujer"){ 655 + (9.6 * Peso) + 
    (1.8 * Altura) - (4.7 * Edad) }
  else{ 66 + (13.7 * Peso) + (5 * Altura) - 
        (6.8 * Edad) }
  } 
TMB("mujer",170,57,32)
```
...
calcularTMB <- function(sexo, peso, altura, edad) {
  ...
  # Retornar el resultado
  #return(tmb)
 return(paste("Tu TMB es", tmb,"kcal/día — no deberías consumir menos que esto."))
}
```
---
---
